...
# visualize_subimages_and_annotations(train_dataset, 30587)
# visualize_images_and_annotations('_annotations.coco.json', 'dataset/train/', 1223)
...
def convert_and_draw(ax, sub_image, target):
    ...
    array = np.array(target["labels"])
    # Draw bounding boxes
    if len(array) != 1 or array != [0]:
